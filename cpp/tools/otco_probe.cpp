...
void FillData(emp::block data0[kLength], emp::block data1[kLength]) {
  for (uint64_t i = 0; i < kLength; ++i) {
    data0[i] = Block(0x1000000000000000ULL | i, 0x0000000000000100ULL | i);
    data1[i] = Block(0x2000000000000000ULL | i, 0x0000000000000200ULL | i);
  }
}

void FillChoices(bool choices[kLength]) {
  const bool pattern[kLength] = {false, true, true, false,
                                 true, false, false, true};
  for (int i = 0; i < kLength; ++i) choices[i] = pattern[i];
}
...
void VerifyRecv(const emp::block out[kLength],
  ...
  for (int i = 0; i < kLength; ++i) {
    ...
    if (std::memcmp(&out[i], &expected, sizeof(emp::block)) != 0) {
      throw std::runtime_error("OTCO recv output mismatch");
    }
  }
}
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    std::fprintf(stderr, "otco_probe: %s\n", e.what());
    return 1;
  }
  return 0;
}
