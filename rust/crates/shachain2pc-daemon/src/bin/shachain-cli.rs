...
fn usage(program: &str) -> String {
    format!(
        "usage: {program} --control-file <path> status|channels|jobs|config <workers|precompute|max-ram-mb> <value>|channel enable <id> [precompute ssp-target cap]|channel disable <id>|reveal <channel> <index> <expected-next> [--allow-seed-reveal]"
    )
}
