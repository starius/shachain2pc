...
### Historical Rust/Rust comparison (50 ms emulated RTT)

This row was measured before Rust was moved to the current AG2PC/SoftSpoken
backend. It is kept only as a historical baseline for the older Rust WRK17/C2PC
backend and should not be used as the current Rust performance claim. Re-run this
measurement after the AG2PC transition when comparing current Rust/Rust,
C++/C++, and cross-mode performance.
...
Historically this was about 3.63 pre-reveal secrets/s and 19.79 reveal
secrets/s. Compared to the C++/C++ 1024 recursive row above, the older Rust
backend was about 2.2x slower on pre-reveal per secret (0.275 s vs 0.125 s) and
used about 3.0x peak RSS (737 MB vs 250 MB). The reveal term was essentially the
same because it was dominated by one 50 ms round trip per secret.
