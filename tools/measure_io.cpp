// Measure per-phase bytes sent by one party for a given index, summed across
...
// bandwidth scales with the generated circuit's AND-gate count. Run like
// demo/party (two processes).
...
int main(int argc, char** argv) {
  if (argc < 4) {
    std::fprintf(stderr, "usage: %s <1|2> <port> <I_hex> [peer_ip]\n", argv[0]);
    return 2;
  }
  ...
  try {
    ...
    index = shachain2pc::util::FromHexU48(argv[3]);
  } catch (const std::exception& e) {
    std::fprintf(stderr, "measure_io: %s\n", e.what());
    return 1;
  }
  const char* peer = argc > 4 ? argv[4] : "127.0.0.1";

  shachain2pc::protocol::Circuit c;
  std::vector<int> gate_arr;
  try {
    c = shachain2pc::run::BuildCircuitForIndex(index);
    gate_arr = shachain2pc::run::ToEmpGateArray(c);
  } catch (const std::exception& e) {
    std::fprintf(stderr, "measure_io: %s\n", e.what());
    return 1;
  }
  ...
  try {
    shachain2pc::run::ExchangeCircuitDigest(
        io, party, shachain2pc::run::CircuitDigest(c, gate_arr));
    io->counter = 0;  // keep the setup measurement focused on EMP traffic.
  } catch (const std::exception& e) {
    delete io;
    std::fprintf(stderr, "measure_io: %s\n", e.what());
    return 1;
  }
  BristolFormat cf(c.num_gate(), c.num_wire, c.n1, c.n2, c.n3, gate_arr.data());
  int ands = 0;
